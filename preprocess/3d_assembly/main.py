...
#  
# pflux0=float(common.read_column('pflux0',0,ctrl_lines))
# pmass0=float(common.read_column('pmass0',0,ctrl_lines))
# decay0=float(common.read_column('decay0',0,ctrl_lines))

# second_decay0=float(common.read_column('second_decay0',0,ctrl_lines))
# second_pmass0=float(common.read_column('second_pmass0',0,ctrl_lines))
...
if ( flag_grid != 'use'):
    ...
    points, vertices, mesh, sources, sinks, kappas,tdens0_functions= ex_grid.example_grid(flag_grid,length,extra_grid)    
    ...
    flags=np.int_(mesh.element_attributes)
...
while ( (time <= tmax) and 
    ...
    if ( str(flag_source) == 'use'):
        cell.readfromfile(extra_source,source_tria)
        steady_source = True
    ...
    if ( str(flag_sink) == 'use'):
        cell.readfromfile(extra_sink,sink_tria)
        steady_sink = True
