module SparseMatrix
  ...
  !>----------------------------------------------------------------------
  type, extends(gen_spmat),public :: spmat
...
contains
  ...
  subroutine init_spmat(this, lun_err, &
    ...
    if (this%storage_system .eq. 'ssr') this%is_symmetric=.true.
...
1010 format(2i15,1pe24.16)
  ...
  !>
  !> usage: call var%csr2ssr(lun_err)
  !>
  !> where:
  ...
  subroutine csr2ssr(this, lun_err)
    ...
    call ssr%init(lun_err,nrow,ncol, nterm_out, &
         this%storage_system,this%dim_kernel,is_symmetric=.true.)
...
!!$    
  ...
  subroutine transpose_spmat(this, lun_err)
    use Globals
    implicit none
    ! Input variables
    class(spmat), intent(inout) :: this
    ...
    ! wok array
    allocate (IW(this%ncol+1),stat=res) 
...
!!$    end do
  ...
  subroutine add_row(this,lun_err,vec_w)
    ...
    !  Al inputs included the spmat format
    !
    !
    !  Licensing:
    !
