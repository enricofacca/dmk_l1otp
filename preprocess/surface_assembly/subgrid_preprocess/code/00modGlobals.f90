...
101 format(5(1x,a))
  ...
  subroutine ortogonalize(dim,nvectors,vectors,x)
   ...
   !>-------------------------------------------------------------
  !> Procedure that computes the projection operator
  !> P = Id - t3*t3^T
  !> and applis it to project a 3D vector of data on
  !> the tangent plane orthogonal to t3
  !> 
  !> usage:
  !>     call tan_proj(t3,data,result)
  !>
  !> where:
  !> \param[in ] t3   -> real(double). dim=3 normal vector
  !>                     It defines the correct plane for the projection
  !> \param[in ] data   -> real(double). dim=3 data array
  !> \param[out] result -> real(double). dim=3 tangent data array
  !>
  !<-------------------------------------------------------------
  subroutine tan_proj(t3, data, res_proj)
    implicit none
    ! vars
    real(kind=double), intent(in) :: t3(3), data(3)
    real(kind=double), intent(out) :: res_proj(3)
    ! local vars
    integer :: i,j
    real(kind=double) :: proj_mat(3,3)

    do i=1,3
       do j=1,3
          proj_mat(i,j) = (i/j)*(j/i) - t3(i)*t3(j)
       end do
    end do
    res_proj = matmul(proj_mat,data)

  end subroutine tan_proj
