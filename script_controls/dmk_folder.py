...
import dmk_folder_structure as dmk 
...
file.close()
globals_repo=os.path.normpath(os.path.abspath(repo+'../globals/'))
linalg_repo=os.path.normpath(os.path.abspath(repo+'../linear_algebra/'))
geom_repo=os.path.normpath(os.path.abspath(repo+'../geometry/'))
fem_repo=os.path.normpath(os.path.abspath(repo+'../p1galerkin/'))
#
# series of proagram exectute by this script
#

scale_timedata_program=(globals_repo+'/build/tools/timedata/scale_timedata.out')
multiply_timedata_program=(globals_repo+'/build/tools/timedata/multiply_timedata.out')
interpolate_timedata_program=(geom_repo+'/build/tools/interpolate_timedata.out')
timedata2vtk_program=(geom_repo+'/build/tools/timedata2vtk.out')
eval_gradient_program=(fem_repo+'/build/tools/eval_gradient.out')
refine_program=(geom_repo+'/build/tools/refine_grid.out')

interpolate_neumann_program= (geom_repo+'/build/tools/interpolate_neumann.out')
project_timedata_program=(geom_repo+'/build/tools/project_timedata/project_timedata.out ')
buildrhs_program= os.path.normpath(fem_repo+'/build/tools/build_p1_rhs.out') 
dmk_program=os.path.normpath(repo+'/build/tools/dmk_p1p0_from_file.out')

import sys
sys.path.append(str(repo))
sys.path.append(str(repo)+'/tools/preprocess/assembly')
import main as input_examples
import common as common
path_td=os.path.normpath(
    os.path.abspath(
        str(repo)+'../globals/python/timedata'))
...
def vtk(folder_name,flag_option,flag_data):
    ...
    if ( flag_option == 'o'):
        if ( flag_data == 'tdens') or ( flag_data == 'all'): 
            command=(str(timedata2vtk_program)      + ' ' +
                     str(grid_path)    + ' ' +
...
def extradata(folder_name,flag_option):
    ...
    if ( flag_option == 'o'):
        ...
        command=(str(eval_gradient_program)      + ' ' +
                 str(subgrid_path) + ' ' +
                 str(folder.result)+'/opt_pot.dat' + ' ' +
        ...
        command=(str(interpolate_timedata)      + ' ' +
                 subgrid_path      + ' ' + 
                 parent_path      + ' ' +
                 str(folder.result)+'/opt_tdens.dat'+' ' +
        ...
        command=(str(muliply_timedata_program)      + ' ' +
                 str(folder.result)+'/opt_tdens_inter.dat'+' ' +
                 str(folder.result)+'/opt_grad.dat' +' ' +
        ...
        # Reverse Vel
        command=(str(scale_timedata_program)      + ' ' +
                 str(folder.result)+'/temp.dat'+' ' +
                 ' -1.0  ' +
        ...
        command=(str(multiply_timedata_program)      + ' ' +
                 str(folder.result)+'/opt_tdens.dat'+' ' +
                 str(folder.result)+'/opt_grad_prj.dat' +' ' +
                 str(folder.result)+'/temp.dat')
        os.system(command)
    ...
    if ( flag_option == 'a'):
        ...
        #
        program=(fem_repo+
                 'build/tools/timedata/eval_gradient/eval_gradient.out')
        command=(str(program)      + ' ' +
                 str(subgrid_path) + ' ' +
                 str(folder.result)+'/pot.dat' + ' ' +
        ...
        # average gradient of grid tdens
        program=(geom_repo+
                 'build/tools/project_timedata/project_timedata.out')
        command=(str(program)      + ' ' +
                 str(folder.result)+'/grad.dat'+ ' ' +
                 str(subgrid_path) + ' ' +
                 str(grid_path)    + ' ' +
...
def assembly(folder_name,file_ctrl):
    ...
    folder=dmk.dmk_folder_structure(folder_name)
    folder.mkdirs()
    # clean dat file to avoid confusions
    flist = os.listdir( folder.input )
    for item in flist:
        if item.endswith(".dat"):
            os.remove( os.path.join( folder.input, item ) )
    ...
    #
    # call assembler for grid-defined variables
    #
    input_examples.assembly_inputs(str(absctrl),str(folder.input),str(folder.input_vtk))

    #
    # Build subgrid and parent (grid/subgrid relation).
    # Flag 1 indicates that we reorder subgrid nodes
    #
    click.echo('BUILD SUBGRID')
    
    command=(refine_program + ' ' +
             str(folder.input)+'/grid.dat' + ' ' +
             '1 1'+' '+
             str(folder.input)+'/subgrid.dat'+' '+
             str(folder.input)+'/parent.dat')
    os.system(command)
    click.echo('DONE\n')
    
    #
    # project rhs related varible (forcing dirac dirichlet  )
    # from grid to subgrid
    #
    click.echo('INTERPOLATE RHS VARIABLE ON SUBGRID')
    click.echo('INTERPOLATE FORCING ON SUBGRID')
    command=(interpolate_timedata_program + ' ' +
             str(folder.input)+'/parent.dat'+ ' '+ 
             str(folder.input)+'/forcing.dat' + ' ' +
             str(folder.input)+'/forcing_subgrid.dat')
    os.system(command)
    click.echo('INTERPOLATE DIRAC ON SUBGRID')
    command=(interpolate_timedata_program + ' ' +
             str(folder.input)+'/parent.dat'+ ' '+ 
             str(folder.input)+'/dirac_forcing.dat' + ' ' +
             str(folder.input)+'/dirac_forcing_subgrid.dat'+' ' 
             'dirichlet')
    os.system(command)
    click.echo('INTERPOLATE DIRICHLET ON SUBGRID')
    command=(interpolate_timedata_program + ' ' +
             str(folder.input)+'/parent.dat'+ ' '+ 
             str(folder.input)+'/dirichlet.dat' + ' ' +
             str(folder.input)+'/dirichlet_subgrid.dat'+' ' 
             'dirichlet')
    os.system(command)
    click.echo('INTERPOLATE NEUMANN ON SUBGRID')
    
    command=(interpolate_neumann_program + ' ' +
             str(folder.input)+'/parent.dat'+ ' '+
             str(folder.input)+'/neumann.dat' + ' ' +
             str(folder.input)+'/neumann_subgrid.dat')
    os.system(command)
    click.echo('DONE\n')


    #
    # assembly rhs on grid and subgrid
    #
    # get controls of balancing
    ctrl = open(file_ctrl, "r")
    ctrl_lines = ctrl.readlines()

    flag_correction=int(common.read_column('flag_correction',0,ctrl_lines))
    tolerance=float(common.read_column('flag_correction',1,ctrl_lines))
    ctrl.close()
    click.echo('BUILD RHS SUBGRID')
   
    command=(buildrhs_program + ' ' +
             str(folder.input)+'/grid.dat'+ ' '+
             str(folder.input)+'/forcing.dat' + ' ' +
             str(folder.input)+'/dirac_forcing.dat' + ' ' +
             str(folder.input)+'/dirichlet.dat' + ' ' +
             str(folder.input)+'/neumann.dat' + ' ' +
             str(folder.input)+'/rhs_grid.dat'+ ' ' +
             str(tolerance) + ' ' +
             str(flag_correction))
    os.system(command)
    click.echo('DONE\n')
    ...
    command=(buildrhs_program + ' ' +
             ...
             str(folder.input)+'/dirichlet_subgrid.dat' + ' ' +
             str(folder.input)+'/neumann_subgrid.dat' + ' ' +
             str(folder.input)+'/rhs_subgrid.dat'+ ' ' +
             str(tolerance) + ' ' +
             str(flag_correction))
    os.system(command)
    click.echo('DONE\n')
...
def run(folder_name,ctrlfile):
    """Run simulation given folder with input data and control file"""
    folder=dmk.dmk_folder_structure(folder_name)
    ...
    #
    # automatically detect with program should be used
    # We also copy the source file
    #
    file = open(str(folder.input)+'/rhs_subgrid.dat', 'r')
    dim = int(file.read().strip()[0])
    file.close()
    
    # if ( dim==1):
    #     program=repo+'/dmkp1p0/muffa.out'
        
    #     file=repo+'/src_dmk/97modDmkP1P0.f90'
    #     filecp=folder.input+'/97modDmkP1P0.f90'
    #     copyfile(os.path.abspath(file),os.path.abspath(filecp))
        
    #     file=repo+'/dmkp1p0/main.f90'
    #     filecp=folder.input+'/main.f90'
    #     copyfile(os.path.abspath(file),os.path.abspath(filecp))
    # else:
    #     program=repo+'/shape_optimization/muffa.out'

    #     file=repo+'/src_dmk/98modDmkSaintVenantP1P0.f90'
    #     filecp=folder.input+'/98modDmkSaintVenantP1P0.f90'
    #     copyfile(os.path.abspath(file),os.path.abspath(filecp))
        
    #     file=repo+'/shape_optimization/main.f90'
    #     filecp=folder.input+'/main.f90'
    #     copyfile(os.path.abspath(file),os.path.abspath(filecp))
    # print( 'Running program :'+str(dim)+str(program) )
    # print( 'Inputs take from:'+str(folder.path) )
    ...
    #
    # mov into assembly folder and run exectution
    #
    cwd=os.getcwd()
    os.chdir(folder.abspath)
    print(os.getcwd())
    os.system(dmk_program)
    os.chdir(cwd)
